...
function getDomainBeid() {
	...
	$system_store = mysqld_select ( 'SELECT id,isclose FROM ' . table ( 'system_store' ) . " where (`website`=:website1 or `website`=:website2) and `deleted`=0 ", array (
			":website1" => WEB_WEBSITE,
			":website2" => 'www.' . WEB_WEBSITE 
	) );
	
	if (empty ( $system_store ['id'] )) {
		if (! empty ( $_GP ['beid'] )) {
			$system_store = mysqld_select ( 'SELECT id,isclose FROM ' . table ( 'system_store' ) . " where `id`=:id  and `deleted`=0", array (
					":id" => $_GP ['beid'] 
			) );
			if (empty ( $system_store ['id'] )) {
				message ( "未找到相关店铺" );
...
function getStoreBeid($beid) {
	$system_store = mysqld_select ( 'SELECT * FROM ' . table ( 'system_store' ) . " store  where store.id=:id and `deleted`=0", array (
			":id" => $beid 
	) );
	return $system_store;
}
...
function keyexchange_set_key($evalue) {
	...
	if (! empty ( $evalue )) {
		...
		$has_key_exchange = mysqld_select ( "SELECT * FROM " . table ( 'key_exchange' ) . " WHERE ekey = :ekey  and beid=:beid ", array (
				':ekey' => $ekey,
				':beid' => $_CMS ['beid'] 
		) );
...
function keyexchange_check() {
	mysqld_query ( 'delete from ' . table ( 'key_exchange' ) . ' where createtime<:createtime and beid=:beid', array (
			':createtime' => TIMESTAMP - (1000 * 60 * 5),
			':beid' => $_CMS ['beid'] 
	) ); // 五分钟过期
}
function keyexchange_get_key($ekey) {
	...
	if (! empty ( $ekey )) {
		keyexchange_check ();
		$key_exchange = mysqld_select ( "SELECT * FROM " . table ( 'key_exchange' ) . " where ekey=:ekey and beid=:beid", array (
				':ekey' => $ekey,
				':beid' => $_CMS ['beid'] 
		) );
		if (! empty ( $key_exchange ['ekey'] )) {
			$evalue = $key_exchange ['evalue'];
			mysqld_delete ( 'key_exchange', array (
					'ekey' => $ekey,
					'beid' => $_CMS ['beid'] 
			) );
...
function order_finish($type, $paytype, $orderid) {
	$order = mysqld_select ( "SELECT * FROM " . table ( 'eshop_order' ) . " WHERE id =:id and uniacid=:uniacid limit 1", array (
			':id' => $orderid,
...
function goldorder_finish($orderid) {
	$order = mysqld_select ( "SELECT * FROM " . table ( 'eshop_member_log' ) . " WHERE id = :id and uniacid=:uniacid limit 1", array (
			':id' => $orderid,
			":uniacid" => $GLOBALS ['_CMS'] ['beid'] 
	) );
	if (empty ( $order ['id'] )) {
...
function isWeixinPayFinish($out_trade_no, $ordertype) {
	$payment = mysqld_select ( "SELECT id,configs FROM " . table ( 'payment' ) . " WHERE code='wechat' and beid=:beid limit 1", array (
			":beid" => $GLOBALS ['_CMS'] ['beid'] 
	) );
	if (empty ( $payment ['id'] )) {
		return false;
	}
...
function isAliPayFinish($out_trade_no, $ordertype) {
	$payment = mysqld_select ( "SELECT id,configs FROM " . table ( 'payment' ) . " WHERE code='alipay' and beid=:beid limit 1", array (
			":beid" => $GLOBALS ['_CMS'] ['beid'] 
	) );
	...
	$sysParams ["biz_content"] = json_encode ( array (
