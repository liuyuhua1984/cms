...
function globaPriveteSystemSetting() {
	...
	if (empty ( $system_config_cache ['value'] )) {
		...
		if (! empty ( $system_config_cache ['name'] )) {
			mysqld_update ( 'system_config', array (
					'value' => serialize ( $config ) 
			), array (
					'name' => 'system_config_cache' 
			) );
		} else {
			mysqld_insert ( 'system_config', array (
					'name' => 'system_config_cache',
					'value' => serialize ( $config ) 
			) );
		}
...
function refreshSystemSetting($arrays) {
	...
	if (is_array ( $arrays )) {
		foreach ( $arrays as $cid => $cate ) {
			$config_data = mysqld_selectcolumn ( 'SELECT `name` FROM ' . table ( 'system_config' ) . " where `name`=:name", array (
					":name" => $cid 
			) );
			if (empty ( $config_data )) {
				mysqld_delete ( 'system_config', array (
						'name' => $cid 
				) );
				$data = array (
						'name' => $cid,
...
function globalBeSetting($beid, $groupkey) {
	...
	$config = array ();
	$system_config_cache = mysqld_select ( 'SELECT * FROM ' . table ( 'config' ) . " where `name`='system_config_cache' and `beid`=:beid and `group`=:group", array (
			":beid" => $beid,
			':group' => $groupkey 
	) );
	if (empty ( $system_config_cache ['value'] )) {
		$configdata = mysqld_selectall ( 'SELECT * FROM ' . table ( 'config' ) . " where `beid`=:beid and `group`=:group", array (
				":beid" => $beid,
				':group' => $groupkey 
		) );
		foreach ( $configdata as $item ) {
...
function refreshBeSetting($beid, $arrays, $groupkey) {
	...
	if (is_array ( $arrays )) {
		foreach ( $arrays as $cid => $cate ) {
			$config_data = mysqld_selectcolumn ( 'SELECT `name` FROM ' . table ( 'config' ) . " where `name`=:name and `beid`=:beid and `group`=:group", array (
					":name" => $cid,
					":beid" => $beid,
					':group' => $groupkey 
			) );
