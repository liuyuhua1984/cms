...
$weburl = urlencode ( WEBSITE_ROOT . create_url ( 'mobile', array (
		'act' => 'dingtalk',
		'do' => 'fastlogin' 
) ) );
...
if (! empty ( $_GP ['code'] )) {
	...
	if (! empty ( $dingtalk_openid )) {
		$base_member = mysqld_select ( "SELECT openid FROM " . table ( 'base_member' ) . " WHERE dingtalk_openid=:dingtalk_openid and beid=:beid ", array (
				':dingtalk_openid' => $dingtalk_openid,
				":beid" => $_CMS ['beid'] 
		) );
		
		$eshop_member = mysqld_select ( "SELECT * FROM " . table ( 'eshop_member' ) . " WHERE openid=:openid and uniacid=:uniacid ", array (
				':openid' => $base_member ['openid'],
				":uniacid" => $_CMS ['beid'] 
		) );
		if (empty ( $eshop_member ['isagent'] ) || empty ( $eshop_member ['status'] )) {
			message ( "登录失败！您不是分销员或者状态不可用!", create_url ( 'mobile', array (
					'act' => 'shopwap',
					'do' => 'logout' 
			) ), "error" );
		}
		if (! empty ( $base_member ['openid'] )) {
			...
			message ( "登陆成功！", create_url ( 'mobile', array (
					'act' => 'shopwap',
					'do' => 'membercenter' 
			) ), 'success' );
		} else {
